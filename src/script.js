...
function setuptStartingCards() {
    ...
    for (let j = 0; j < 4; j++) {
        ...
        for (let i = 0; i < arrPlayer_[j].cards_.length; i++) {
            ...
            let myElem = document.getElementsByClassName("Player" + (j + 1)+"-hand")[0];
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (result.error === "WrongColor") {
                console.log("error  --> wrongcolor")
                showErrorToSelectCard(true);
                return false;
            } else if (result.error === "Draw4NotAllowed") {
                wild = lastWild;
                console.log("lastWild -->" + lastWild+"--> draw4notallowed");
                showErrorToSelectCard(true);
                return false;
            } else {
                console.log('background color -->' + lastWild+" removed");
            ...
            console.log("HTTP-Error:  response not OK" + response.status);
            return false;
        }
    } catch (e) {
        alert("No internet connection: " + e)
...
function saveResponseFromServerAfterSetPlayersHandsAndScores(response, playerNumber) {
    ...
    if (cardsPlayerToSetHand.length === 0 && lastPlayer === fieldnamenList[playerNumber - 1]) {
        ...
        let myElem = document.getElementById("hand-player" + playerNumber);
        const img = document.createElement("img");
        img.id = "congratulation-label";
        img.classList.add('heartbeat');
        img.src = "src/img2/Congratulations.png"
        myElem.appendChild(img);
    }
}
...
for (let i = 0; i < 4; i++) {
    ...
    document.getElementsByClassName("card-body hand")[i].addEventListener("click", function (event) {
        if (event.target.nodeName == 'IMG') {
            ...
            console.log("NO imagin in event.target!!!")
        }
    });
    
}
/**************************************Ende of big for loop****************************************************************************/
    ...
    for (let i = 0; i < arrCardActivePlayer.length; i++) {
        ...
        if (pileColor == "Black") {    
            ...
            if (arrCardActivePlayer[i].dataset.color == wild) {
                console.log("card with same color");
                return true;
            }
        }
